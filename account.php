...
<!--2.History page of user account-->
...
<!--3.Leader board of user account-->
...
if (@$_GET['q'] == 1) {
    ...
    //This show quizes which have status "enabled" by admin
...
<tr><td style="vertical-align:middle"><b>S.N.</b></td><td style="vertical-align:middle"><b>Name</b></td><td style="vertical-align:middle"><b>Total question</b></td><td style="vertical-align:middle"><b>Correct Answer</b></td><td style="vertical-align:middle"><b>Wrong Answer</b></td><td style="vertical-align:middle"><b>Total Marks</b></td><td style="vertical-align:middle"><b>Time limit</b></td><td style="vertical-align:middle"><b>Action</b></td></tr>';
    ...
    while ($row = mysqli_fetch_array($result)) {
        ...
        //when user filled quiz then we have view result which shows users total questions,coorect,wrong answers and unattempted answer numbers..
  ...
  <td style="vertical-align:middle"><b><a href="account.php?q=quiz&step=2&eid=' . $eid . '&n=1&t=' . $total . '&start=start" class="btn" style="color:#FFFFFF;background:darkgreen;font-size:12px;padding:7px;padding-left:10px;padding-right:10px"><span class="glyphicon glyphicon-new-window" aria-hidden="true"></span>&nbsp;<span><b>Start</b></span></a></b></td></tr>';
            ...
            //when user doesn't attempted quiz then it shows start button after clicking on it user start attempted its quiz in current situation of filling auiz it shows status as ongoing.
  ...
  <td style="vertical-align:middle"><b><a href="account.php?q=quiz&step=2&eid=' . $eid . '&n=1&t=' . $total . '&start=start" class="btn" style="margin:0px;background:darkorange;color:white">&nbsp;<span class="title1"><b>Continue</b></span></a></b></td></tr>';//continue...
            } else {
                echo '<tr style="color:darkgreen"><td style="vertical-align:middle">' . $c++ . '</td><td style="vertical-align:middle">' . $title . '&nbsp;<span title="This quiz is already solve by you" class="glyphicon glyphicon-ok" aria-hidden="true"></span></td><td style="vertical-align:middle">' . $total . '</td><td style="vertical-align:middle">+' . $correct . '</td><td style="vertical-align:middle">-' . $wrong . '</td><td style="vertical-align:middle">' . $correct * $total . '</td><td style="vertical-align:middle">' . $time . '&nbsp;min</td>
  <td style="vertical-align:middle"><b><a href="account.php?q=result&eid=' . $eid . '" class="btn" style="margin:0px;background:darkred;color:white">&nbsp;<span class="title1"><b>View Result</b></span></a></b></td></tr>';//view result...
            }
        }
    }
...
if (@$_GET['q'] == 'quiz' && @$_GET['step'] == 2 && isset($_SESSION['6e447159425d2d']) && $_SESSION['6e447159425d2d'] == "6e447159425d2d" && isset($_GET['endquiz'])== 'end') {
    ...
    //when user attempted any quiz fully then in database its quiz status updated to finished....
    ...
    $q = mysqli_query($con, "UPDATE history SET status='finished' WHERE username='$_SESSION[username]' AND eid='$_GET[eid]' ") or die('Error197');
        $q = mysqli_query($con, "SELECT * FROM history WHERE eid='$_GET[eid]' AND username='$_SESSION[username]'") or die('Error156');
                ...
                //fetches quiz score and status
                while ($row = mysqli_fetch_array($q)) {
                    $s = $row['score'];
                    $scorestatus = $row['score_updated'];
                }
                //if it is false then set to the true
                 if($scorestatus=="false"){
                    ...
                    //when user attempted quiz its adds score and save into database.
...
//when user attempt to start quiz if quiz is enabled from admin side and its staus is ongoing then user is able to attempt quiz otherwise quiz status is updated to finshed....
if (@$_GET['q'] == 'quiz' && @$_GET['step'] == 2 && isset($_GET['start']) && $_GET['start'] == "start" && (!isset($_SESSION['6e447159425d2d']))) {
    ...
    if (mysqli_num_rows($q) > 0) {
        ...
        //if status is ongoing and quiz status is enabled then user is able to attempt quiz....
        ...
        //else status updated to finished...
        else {
                $q = mysqli_query($con, "UPDATE history SET status='finished' WHERE username='$_SESSION[username]' AND eid='$_GET[eid]' ") or die('Error197');
        $q = mysqli_query($con, "SELECT * FROM history WHERE eid='$_GET[eid]' AND username='$_SESSION[username]'") or die('Error156');
                while ($row = mysqli_fetch_array($q)) 
    ...
    {
        //otherwise status is going to false in score_updated and quiz status is ongoing because user is attempted quiz....
        $time = time();
...
//it fetches timestamp and status from history which is ongoing and false if user is filling quiz and when user submit it quiz an alert appears in which we need to write yes to complete quiz....
if (@$_GET['q'] == 'quiz' && @$_GET['step'] == 2 && isset($_SESSION['6e447159425d2d']) && $_SESSION['6e447159425d2d'] == "6e447159425d2d") {
    ...
    if (mysqli_num_rows($q) > 0) 
    {
        ...
        if ($status == "ongoing" && $remaining > 0 && $qstatus == "enabled") 
        {
            ...
            //qform is for quiz form and sbutton-set
            //below has function for end quiz message and time count down.
            $remaining = (($ttime * 60) - ((time() - $time)));
            echo '<script>
...
<br />';
        //print option and choose options...
            ...
            //it displays option that user choose as answer....
            if (strlen($ans) > 0) 
            {
            ...
            echo '</div>';
            //this is about set lock and reset options....
            ...
            //print lock and reset button and next question 
            ...
            //it prints footer which is in quiz form.
            //that footer indicate questins number......1,2,3...
            for ($i = 1; $i <= $total; $i++) 
            {
        ...
        //after attempting quiz its status is updated to finished.....
        else 
        {
            unset($_SESSION['6e447159425d2d']);
            $q = mysqli_query($con, "UPDATE history SET status='finished' WHERE username='$_SESSION[username]' AND eid='$_GET[eid]' ") or die('Error197');
...
//prints result after attempting quiz......
...
<center><h1 class="title" style="color:#660033">Result</h1><center><br /><table class="table table-striped title1" style="font-size:20px;font-weight:1000;">';
    ...
    //for print overall score...
        ...
        //print detailed analysis of quiz correct answer questions answers......
        ...
        while ($row = mysqli_fetch_array($q)) 
        {
            ...
            //if not attempted then unanswered print..
            else 
            {
            ...
            //if answer is not unanswered then print our answer and correct answer....
            ...
            //if it is unanswered then print questions and answers.....
...
//prints total questions correct wrong score and action
if (@$_GET['q'] == 2) 
{
...
//leaderboard......
//prints all users rank and score with details.....
